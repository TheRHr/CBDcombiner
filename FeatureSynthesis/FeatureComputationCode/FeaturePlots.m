...
viewwin  =[DeltaB and.timestamps(1) DeltaBand.timestamps(end)];
clusterfig = figure('visible','off');
    ...
    imagesc(DeltaBand.timestamps,log2(DeltaBand.freqs),log10(DeltaBand.spec))
        ...
        %clim([3.5 6.5])
        clim([min(mu)-2*max(sig) max(mu)+2*max(sig)])
        xlim(viewwin)
        colorbar('east')
	...
	subplot(9,1,3)
        ...
        set(gca,'YTickLabel',{'1','2','4','8','16','32','64','128'})
        clim([3.5 6.5])
        %clim([min(mu_th)-2*max(sig_th) max(mu_th)+2*max(sig_th)])
        xlim(viewwin)
        %colorbar('east')
    ...
    subplot(9,1,4)
        ...
        set(gca,'YTickLabel',{'1','2','4','8','16','32','64','128'})
        clim([3.5 6.5])
        %clim([min(mu_b)-2*max(sig_b) max(mu_b)+2*max(sig_b)])
        xlim(viewwin)
        %colorbar('east')
