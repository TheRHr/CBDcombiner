...
colnames = {'Ratnumber','StudyDay','posttrial (0 = presleep)','Condition (1=OR, 2=OD, 3=HC)','Treatment (0=VEH, 1=CBD)','Epoch (seconds)','State (1=wake, 3=nrem, 4=nrem2rem, 5=rem)','DeltaHPC','DeltaPFC','ThetaHPC','ThetaPFC','EMG'};
samplingrate = 2500; 
TargetSampling = 1250;
...
for i =5:length(directories)-3
    ...
    for j =3:length(ratDirs)-11
        ...
        for k = 3:length(SDdirs)-5
            ...
            %load in the raw trial data and states per epoch
            if i < 5
            ...
            elseif i == 5
                HPCload = load(append(trialPath,"HPC_100_CH14_0.continuous.mat"));
                HPC =HPCload.HPC;
                PFCload = load(append(trialPath,"PFC_100_CH12_0.continuous.mat"));
                PFC =PFCload.PFC;
   
            elseif i == 6
                HPCload = load(append(trialPath,"HPC_100_CH15_0.continuous.mat"));
                HPC =HPCload.HPC;
                PFCload = load(append(trialPath,"PFC_100_CH12_0.continuous.mat"));
                PFC =PFCload.PFC;
            ...
            load(append(trialPath, trial,"-states_ES.mat"));
            ...
            lfpFeatures(:,7) = states(1:length(EMG));
            lfpFeatures(:,8) = DeltaBandHPC.data;
            lfpFeatures(:,9) = DeltaBandPFC.data;
            lfpFeatures(:,10) = ThetaBandHPC.data;
            lfpFeatures(:,11) = ThetaBandPFC.data;
            lfpFeatures(:,12) = EMG;
